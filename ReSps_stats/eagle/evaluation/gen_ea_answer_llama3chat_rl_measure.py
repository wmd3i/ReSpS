...
class OverheadProfiler:
    """Profile overhead of different components during EAGLE generation"""
    def __init__(self):
        self.reset()
    ...
    def end_timing(self):
        ...
        if self.context and self.start_time:
            ...
            self.times[self.context].append(elapsed)
            self.counts[self.context] += 1
            self.context = None
            self.start_time = None
            return elapsed
        return 0.0
    ...
    def time_component(self, component_name):
        ...
        return TimingContext(self, component_name)
    
    def get_stats(self):
        """Get timing statistics for all components"""
        stats = {}
        for component, times_list in self.times.items():
            if times_list:
                stats[component] = {
    ...
    def print_summary(self, title="Overhead Analysis"):
        ...
        # Sort by total time
        sorted_components = sorted(stats.items(), key=lambda x: x[1]['total_time'], reverse=True)
        
        total_measured_time = sum(s['total_time'] for s in stats.values())
        
        print(f"{'Component':<25} {'Count':<8} {'Total(s)':<10} {'Avg(ms)':<10} {'Min(ms)':<10} {'Max(ms)':<10} {'%':<8}")
        print("-" * 80)
        
        for component, stat in sorted_components:
            avg_ms = stat['avg_time'] * 1000
            min_ms = stat['min_time'] * 1000  
            max_ms = stat['max_time'] * 1000
            total_s = stat['total_time']
            percentage = (total_s / total_measured_time * 100) if total_measured_time > 0 else 0
            
            print(f"{component:<25} {stat['count']:<8} {total_s:<10.3f} {avg_ms:<10.2f} {min_ms:<10.2f} {max_ms:<10.2f} {percentage:<7.1f}%")
        
        print("-" * 80)
        print(f"{'TOTAL MEASURED':<25} {'':<8} {total_measured_time:<10.3f} {'':<10} {'':<10} {'':<10} {'100.0%':<8}")
        print(f"{'='*60}")
...
class TimingContext:
    ...
    def __enter__(self):
        self.profiler.start_timing(self.component_name)
        return self
...
def get_model_answers(
    ...
    for _ in range(3):
        ...
        for j in range(len(question["turns"])):
            ...
            # Get parameters from policy - WITH OVERHEAD MEASUREMENT (WARMUP)
            ...
            if online_policy is not None:
                ...
                if is_optimized_policy:
                    ...
                    with overhead_profiler.time_component("RL_Policy_Prediction_Optimized_Warmup"):
                    ...
                    # Determine the timing component name based on policy type
                    if hasattr(online_policy, '__class__') and 'OptimizedSB3' in online_policy.__class__.__name__:
                        if getattr(args, 'optimized_policy_version', 'standard') == 'ofl':
                            timing_component = "RL_Policy_Prediction_OFL_Warmup"
                        else:
                            timing_component = "RL_Policy_Prediction_OptimizedSB3_Warmup"
                    else:
                        timing_component = "RL_Policy_Prediction_Traditional_Warmup"
                    
                    with overhead_profiler.time_component(timing_component):
                        predicted_total_tokens, predicted_depth, predicted_top_k = online_policy.predict_parameters(
                ...
                # Offline RL policy
                with overhead_profiler.time_component("RL_Policy_Prediction_Offline_Warmup"):
                ...
                # Fallback to defaults - no overhead
            ...
            try:
                with torch.no_grad():
                    if args.use_stepwise_rl and online_policy is not None:
                        ...
                        with overhead_profiler.time_component("Model_Generation_Stepwise_RL_Warmup"):
                        ...
                        with overhead_profiler.time_component("Model_Generation_Traditional_Warmup"):
    ...
    for question in tqdm(questions, desc="Processing questions"):
        ...
        for i in range(num_choices):
            ...
            for j in range(len(question["turns"])):
                ...
                # Get parameters from policy - WITH OVERHEAD MEASUREMENT
                full_context = " ".join([msg["content"] for msg in messages])
                ...
                if online_policy is not None:
                    ...
                    if is_optimized_policy:
                        ...
                        if args.online_inference_only:
                            ...
                            with overhead_profiler.time_component("RL_Policy_Prediction_Optimized_Inference"):
                                predicted_total_tokens, predicted_depth, predicted_top_k = online_policy.predict_parameters(
                            ...
                            with overhead_profiler.time_component("RL_Policy_Prediction_Optimized_Training"):
                                predicted_total_tokens, predicted_depth, predicted_top_k = online_policy.predict_parameters(
                        ...
                        if hasattr(online_policy, '__class__') and 'OptimizedSB3' in online_policy.__class__.__name__:
                            if getattr(args, 'optimized_policy_version', 'standard') == 'ofl':
                                timing_component = "RL_Policy_Prediction_OFL_Training" if not args.online_inference_only else "RL_Policy_Prediction_OFL_Inference"
                            else:
                                timing_component = "RL_Policy_Prediction_OptimizedSB3_Training" if not args.online_inference_only else "RL_Policy_Prediction_OptimizedSB3_Inference"
                        else:
                            timing_component = "RL_Policy_Prediction_Traditional_Training" if not args.online_inference_only else "RL_Policy_Prediction_Traditional_Inference"
                    ...
                    # Offline RL policy
                    with overhead_profiler.time_component("RL_Policy_Prediction_Offline"):
                        predicted_total_tokens, predicted_depth, predicted_top_k = rl_policy.predict_parameters(full_context)
                    print(f"Offline RL predicted params: total_tokens={predicted_total_tokens}, depth={predicted_depth}, top_k={predicted_top_k}")
                else:
                    # Fallback to defaults - no overhead to measure
                    predicted_total_tokens = args.total_token
                    predicted_depth = args.depth
                    predicted_top_k = args.top_k
                    # print(f"Using default params: total_tokens={predicted_total_tokens}, depth={predicted_depth}, top_k={predicted_top_k}")
                    ...
                    try:
                        ...
                        if training_mode and online_policy is not None:
                            ...
                            if args.use_stepwise_rl and online_policy is not None:
                                ...
                                with overhead_profiler.time_component("Model_Generation_Stepwise_RL_Training"):
                                    result = model.eagenerate(
                                        torch.as_tensor(input_ids).cuda(),
                                        temperature=temperature,
                                        log=True,
                                ...
                                # Traditional mode: fixed parameters for entire generation
                                with overhead_profiler.time_component("Model_Generation_Traditional_Training"):
                                    output_ids, new_token, idx = model.eagenerate(
                                        torch.as_tensor(input_ids).cuda(),
                                        temperature=temperature,
                                        log=True,
                            ...
                            with torch.no_grad():
                                if args.use_stepwise_rl and online_policy is not None:
                                    ...
                                    with overhead_profiler.time_component("Model_Generation_Stepwise_RL_Inference"):
                                        result = model.eagenerate(
                                            torch.as_tensor(input_ids).cuda(),
                                            temperature=temperature,
                                            log=True,
                                    ...
                                    # Traditional mode: fixed parameters for entire generation
                                    with overhead_profiler.time_component("Model_Generation_Traditional_Inference"):
                                        output_ids, new_token, idx = model.eagenerate(
                                            torch.as_tensor(input_ids).cuda(),
                                            temperature=temperature,
                                            log=True,
                ...
                if not args.use_stepwise_rl and online_policy is not None and not args.online_inference_only:
                    ...
                    with overhead_profiler.time_component("RL_Policy_Update"):
    ...
    overhead_profiler.print_summary("🔍 EAGLE Generation Overhead Analysis")
...
if __name__ == "__main__":
    ...
    # Overhead measurement arguments
    parser.add_argument(
        "--enable-overhead-measurement",
        action="store_true",
        default=True,
        help="Enable detailed overhead measurement for different components (default: enabled)"
    )
    parser.add_argument(
        "--disable-overhead-measurement",
        action="store_true",
        help="Disable overhead measurement to reduce noise in benchmarks"
    )

    args = parser.parse_args()
    
    # Handle overhead measurement setting
    if getattr(args, 'disable_overhead_measurement', False):
        args.enable_overhead_measurement = False
        print("🔧 Overhead measurement disabled via --disable-overhead-measurement flag")
        ...
        overhead_profiler = DisabledProfiler()
    else:
        print("🔧 Overhead measurement enabled - detailed component timing will be collected")
